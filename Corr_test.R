...
if(NCOL(all_depth_csv) > 2){
  all_depth_csv <- all_depth_csv[,-1]
  all_depth_csv <- all_depth_csv[,colSums(all_depth_csv != 0) > 0]

  if(NCOL(all_depth_csv) > 1){
    num_rows = ncol(all_depth_csv)
    colnames(all_depth_csv)

    Cor_matrix = matrix(NA, num_rows, num_rows)
    rownames(Cor_matrix) <- colnames(all_depth_csv)
    colnames(Cor_matrix) <- colnames(all_depth_csv)

  for (r in 1:nrow(Cor_matrix)) {
    for (c in 1:ncol(Cor_matrix)) {
      cor = cor.test(all_depth_csv[,r], all_depth_csv[,c], methods = c("spearman"))$estimate
  ...
  row_counter = 1
  for (first in 1:ncol(all_depth_csv)) {
    ...
    for (second in number:ncol(all_depth_csv)) {
      print(second)
      ...
      cor = cor.test(all_depth_csv[,first],all_depth_csv[,second],methods=c("spearman"))$estimate
  ...
  The_plot <- ggplot(db.3, aes(x = Corr, group = GROUPED, fill = GROUPED, color = GROUPED))+
    scale_x_continuous(breaks = round(seq(min(db.3$Corr), max(db.3$Corr), by = 0.5),1))+
