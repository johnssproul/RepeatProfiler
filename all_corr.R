...
while(NCOL(all_depth_cvs)>0){
  ...
  if(NCOL(current_cvs_samples) > 1){
    ...
    cat('Correlation analysis for one of the refrences couldnt be done because  there is only 1 sample with non-zero depth values for this reference .\n', file = "R_all_correlation_errors.txt",append = TRUE)
    print('Correlation analysis for one of the refrences couldnt be done because  there is only 1 sample with non-zero depth values for this reference')
  }
