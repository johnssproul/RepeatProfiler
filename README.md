...
| -bowtie ""                   | insert  parameters for bowtie2 here between these quotations like:   -bowtieflag "--sensitive-local -no-mixed" .
Allows user to change Bowtie 2 mapping parameters freely without restricting to the options above. If this option is used, all relvant bowtie should be within the quotations as the program will not check for other aligning settings if this was used.                                                                                                              |
