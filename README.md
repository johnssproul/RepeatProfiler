...
This read me tells you what indexes that you will see later when we go to the the specific refrences _output file means.  In short, we refer to your reads paired or unpaired in an index. This makes it easier to name the files and manage output. We convert it back to your read name when analyzing graphs and summary tables. This is just for the folders.  

Output includes the Run_summary.csv table. It contains useful info about the references you used (e.g., length), average coverage, percent coverage, etc. 
...
Each row represents one reference with reads mapped from one sample, so if you had for example 4 pairs of reads/4 unpaired reads, you will have 4 rows with the same reference name and each row will contain the corrsoponding attributes shown 
